...
def data_preprocessing():
    df = pd.read_csv("/opt/airflow/dags/data/email_spam.csv")
    df = cleaning(df)
    df = aggregate(df)
    df = remove_extra_col(df)
    df.to_csv("/opt/airflow/dags/artifacts/cleaned_data.csv", index=False)


def create_artifacts(df):
    vectorizer = TfidfVectorizer()
    vectorizer.fit(df['msg'])
    joblib.dump(vectorizer, "/opt/airflow/dags/artifacts/vectorizer.sav")

    encoder = LabelEncoder()
    encoder.fit(df['type'])
    joblib.dump(encoder, "/opt/airflow/dags/artifacts/encoder.sav")


def prepare_train_data(df):
    vectorizer = joblib.load("/opt/airflow/dags/artifacts/vectorizer.sav")
    encoder = joblib.load("/opt/airflow/dags/artifacts/encoder.sav")
...
def train(X, y):
    ...
    joblib.dump(model, "/opt/airflow/dags/artifacts/model.sav")
...
def train_model():
   df = pd.read_csv("/opt/airflow/dags/artifacts/cleaned_data.csv")
   create_artifacts(df)
